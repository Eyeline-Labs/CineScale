...
class WanVideoPipeline_Pro(BasePipeline):
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None, use_usp=False):
        ...
        pipe = WanVideoPipeline_Pro(device=device, torch_dtype=torch_dtype)
    ...
    def __call__(
        ...
        latents_list.append(latents.clone().detach())
        ...
        for target_size_ in target_size_list:
            ...
            if input_image is not None and self.image_encoder is not None:
                self.load_models_to_device(["image_encoder", "vae"])
                image_emb = self.encode_image(input_image, end_image, num_frames, target_size_[-2] * 8, target_size_[-1] * 8, **tiler_kwargs)
                self.load_models_to_device(["dit", "motion_controller"])
            else:
                image_emb = {}
            ...
            for timestep in self.scheduler.timesteps:
                noise_latent = self.scheduler.add_noise(latents, noise.unsqueeze(0), timestep.unsqueeze(0))
            ...
            for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
                ...
                new_cfg_scale = cfg_scale
                # new_cfg_scale = 6.0
                if new_cfg_scale != 1.0:
                    noise_pred_nega = model_fn_wan_video(
                        self.dit, motion_controller=self.motion_controller,
                        x=latents, timestep=timestep,
                        **prompt_emb_nega, **image_emb, **extra_input,
                    ...
                    noise_pred = noise_pred_nega + new_cfg_scale * (noise_pred_posi - noise_pred_nega)
                else:
                    noise_pred = noise_pred_posi
